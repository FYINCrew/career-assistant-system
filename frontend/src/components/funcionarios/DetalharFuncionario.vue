...
<template>  
        ...
        <div>
            <div class="flex flex-col items-center justify-center gap-4">
                <div class="bg-primary-50 rounded-lg p-6 shadow">
                    <h2 class="text-4xl font-semibold mb-3 text-center text-primary-800">{{ nome }}</h2>
                    <Divider class="w-1/2 mb-4 " />
                    <div class="flex flex-col gap-1 text-xl text-primary-700">
            ...
            <Panel toggleable class=" !bg-gray-50 !border-primary-200 rounded-lg shadow-lg">
                <template #header>
                    <div class="flex items-center justify-between">
                        <span class="text-3xl font-semibold pt-3 pb-3 pl-3">Experiências</span>
                        <Button icon="pi pi-question-circle" class="p-button-rounded p-button-text"
                            v-tooltip.right="'Veja detalhes sobre as experiências do usuário e a pontuação atribuída para cada tecnologia utilizada.'" />
                    </div>
                    ...
                    <AccordionPanel :value="index" v-for="(experiencia, index) in funcionarioDetalhes?.experiencias"
                        ...
                        <AccordionContent>
                            <div class="!text-base !text-gray-700 !leading-relaxed !mb-6 ml-2 ">
                                <strong class=" text-lg text-primary">Descrição</strong>
                                <p class="ml-2 mt-2">{{ experiencia.texto }}</p>
                            </div>

                            <div>
                                <strong class="text-lg text-primary ml-2">Tecnologias</strong>
                                <div class="flex flex-wrap gap-2 mt-1">
                                    <div class="flex flex-col gap-2 mt-1">
                                        <Chip
                                            v-tooltip.right="getClassificacaoPorScore(score.score, 'Pontos não Calculados')"
                                            ...
                                            <Badge class="ml-auto !text-white"
    ...
    <div class="flex justify-end px-10 py-2 ">
        <Button
            ...
            label="Voltar" variant="link" icon="pi pi-angle-left"
            @click=" router.push({ name: 'listarFuncionarios' })" />
        <Button
            ...
            icon="pi pi-calculator" label="Calcular Média"
            @click="calcularScore(Number.parseInt(funcionarioId as string))" />
    </div>
</template>
